...
8. **Safeguards & permissions.** Scoped API keys; per-lane tool allowlists (the estimator has no fetch tools); review required by default (`review: auto` is replay-only); fail-closed gates, held and never silently degraded.
