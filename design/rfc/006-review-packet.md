...
**Uncertain items.** Claims or quantities that the estimator, or the auditor, flagged as uncertain, each with a reason.

**Investor judgment items.** Questions the model recommends a human decide. Each has `question`, `why` (why this is judgment rather than fact), `related_quantities`, and optional `considerations`. Examples from `merger_arb`: whether the acquirer will accept a divestiture remedy, and whether a political objection is likely to become a formal challenge.
...
A claim is one atomic statement, for example "The merger agreement's outside date is 2027-03-01" or "Given the second request, closing before Q4 is unlikely". Each claim carries:
...
- **computed**: produced by deterministic code from extracted claims, with no model involved. Examples: days between two dates, or break fee as a percentage of equity value. The formula and its inputs are recorded.
...
| Dates *stated in* claims (for example "as of the June 5 proxy") match the cited evidence's as_of or content | auditor model | claim flagged, `as_of_ok: false` |
...
- How should the packet show evidence that is *absent*? For example, the S-4 is overdue and that absence is itself informative. Is "absence" a claim type with no citation?
